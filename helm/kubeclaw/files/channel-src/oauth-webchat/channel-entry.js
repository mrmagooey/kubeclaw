...
function isEmailAllowed(email, emailVerified, allowlist) {
...
function parseConfig(sdk) {
  const file = sdk.readEnvFile(ENV_KEYS);
  ...
  if (!publicUrl) {
    sdk.logger.warn('oauth-webchat: OAUTH_WEBCHAT_PUBLIC_URL is required');
    return null;
  }
  if (!oidcIssuer) {
    sdk.logger.warn('oauth-webchat: OAUTH_WEBCHAT_OIDC_ISSUER is required');
    return null;
  }
  ...
  if (!allowedEmails) {
    sdk.logger.warn('oauth-webchat: OAUTH_WEBCHAT_ALLOWED_EMAILS is required');
    return null;
  }
...
function verifyStateCookie(cookie, secret) {
  const dot = cookie.indexOf('.');
  if (dot < 1 || dot === cookie.length - 1) return null;
  const payloadB64 = cookie.slice(0, dot);
  const sigB64 = cookie.slice(dot + 1);
...
const CHAT_HTML = (email) => `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
const LOGIN_HTML = (providerName) => `<!DOCTYPE html>
...
function getSessionFromCookies(cookies, secret) {
...
class OAuthWebchatChannel {
  constructor(config, opts, sdk) {
    this.name = 'oauth-webchat';
    this.capabilities = {
      inboundImages: true,
      outboundMedia: true,
    };
  ...
  async connect() {
    this.server = createServer((req, res) => {
      this.handleRequest(req, res).catch((err) => {
        this.sdk.logger.error({ err, url: req.url }, 'oauth-webchat handler error');
        if (!res.headersSent) {
  ...
  async disconnect() {
    ...
    if (this.server) {
      ...
      this.server = null;
      this.sdk.logger.info('oauth-webchat channel closed');
    }
  }
  ...
  async sendMessage(jid, text) {
    ...
    if (clients.length === 0) {
      this.sdk.logger.debug({ jid }, 'oauth-webchat: no SSE client connected');
      return;
    }
    ...
    for (const client of clients) {
      try {
        ...
        this.sdk.logger.debug({ jid, err }, 'oauth-webchat: SSE write failed');
      }
    }
    this.sdk.logger.info(
  ...
  async handleRequest(req, res) {
    ...
    if (req.method === 'GET' && url.pathname === '/callback') {
      ...
      try {
        ...
        this.sdk.logger.warn({ err }, 'oauth-webchat: token exchange failed');
        res.writeHead(400, { 'Content-Type': 'text/plain' });
  ...
  handleInbound(email, text) {
    ...
    const group = this.opts.registeredGroups()[jid];
    if (!group) {
      this.sdk.logger.debug({ jid }, 'oauth-webchat message from unregistered user');
      return;
    }
    ...
    this.sdk.logger.info({ jid }, 'oauth-webchat message stored');
  }
}

export default function register(sdk) {
  sdk.registerChannel('oauth-webchat', (opts) => {
    const cfg = parseConfig(sdk);
    if (!cfg) return null;
    return new OAuthWebchatChannel(cfg, opts, sdk);
  });
}
