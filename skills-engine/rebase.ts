...
import { BASE_DIR, KUBECLAW_DIR } from './constants.js';
...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, KUBECLAW_DIR, 'state.yaml');
    ...
    try {
      ...
      // Save combined patch
      const patchPath = path.join(projectRoot, KUBECLAW_DIR, 'combined.patch');
      ...
      if (newBasePath) {
        ...
        for (const relPath of trackedFiles) {
          ...
          const tmpSaved = path.join(
            os.tmpdir(),
            `kubeclaw-rebase-${crypto.randomUUID()}-${path.basename(relPath)}`,
          );
