/**
 * e2e tests for Story 22: Attachment delete via HTTP channel.
 *
 * Acceptance criteria:
 *  AC1. Authenticated DELETE → 204; GET afterward → 404.
 *  AC2. No auth → 401.
 *  AC3. Nonexistent filename → 404.
 *  AC4. Path traversal `../etc/passwd` → 400.
 *  AC5. Cross-user filename (bob's file deleted as alice) → 404.
 *
 * LLM-independent.
 *
 * Prerequisites:
...
const NS = 'kubeclaw-e2e-attach-del';
const RELEASE = 'ke2e-attach-del';
const CONTEXT = 'kind-kubeclaw-e2e-istio';
const LOCAL_PORT = 14107; // unique: no other e2e test uses this port
 ...
 */
function rawRequest(method: string, rawPath: string, authHeader?: string): Promise<number> {
...
describe.skipIf(!clusterReachable)(
  'attachment delete via HTTP channel (Story 22)',
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      const valuesDir = mkdtempSync(path.join(tmpdir(), 'ke2e-attach-del-'));
    ...
    it(
      'AC1: authenticated DELETE returns 204 and subsequent GET returns 404',
      async () => {
        expect(
          aliceFilename,
        ...
        expect(delRes.status, 'Authenticated DELETE must return 204').toBe(204);
    ...
    it(
      'AC2: DELETE without Authorization header returns 401',
      async () => {
    ...
    it(
      'AC3: DELETE of nonexistent filename returns 404',
      async () => {
        const res = await fetch(
        ...
        expect(res.status, 'DELETE of nonexistent file must return 404').toBe(404);
      },
      TEST_TIMEOUT,
    );
    ...
    it(
      'AC4: DELETE with path-traversal filename returns 400',
      async () => {
        ...
        expect(status, 'DELETE with path-traversal must return 400').toBe(400);
      },
      TEST_TIMEOUT,
    );
    ...
    it(
      "AC5: alice cannot delete a file uploaded by bob (returns 404)",
      async () => {
        expect(
          bobFilename,
        ...
        expect(
          res.status,
          "DELETE of bob's file as alice must return 404",
        ).toBe(404);
      },
      TEST_TIMEOUT,
    );
