...
describe.skipIf(!K8S_AVAILABLE || SKIP_E2E)(
  ...
  () => {
    it(
      'placeholder: full LLM-driven discover→probe→register→cross-group-isolation flow pending CI cluster',
      () => {
        // ── Workflow under test ────────────────────────────────────────────────
        //
        // Prerequisites:
        ...
        //   credentialed→pending_credential:
        //     src/tool-selection/agent.test.ts
        //   Covers: successful discovery writes tool entry scoped to requesting
        //   groupId; a discovered tool with requiredCredential is registered as
        //   pending_credential (not activated); cross-group isolation enforced at
        //   the storage layer (second group's find_tools does not see first
        //   group's discovered tool).
        //
