...
export async function probeTool(
  ...
  const res = await runner.runProbeToolJob({
    toolSpec: probeSpec,
    input,
    timeoutMs: PROBE_TIMEOUT_MS,
  });

  if (res.egressViolation)
    return { verified: false, reason: 'probe attempted off-allowlist egress' };
  if (!res.ok)
    return {
      verified: false,
      reason: `probe failed: ${res.error ?? 'unknown error'}`,
    };
  if (!res.output || res.output.trim().length === 0)
    return { verified: false, reason: 'probe returned empty output' };
