...
describe('credential gate', () => {
  ...
  it('round-trips a valid approval token', () => {
    ...
    expect(
      verifyApprovalToken(t, 'image_search', 'brave-search', 'nonce123'),
    ).toBe(true);
  });
  ...
  it('rejects a token for a different tool/credential/nonce', () => {
    const t = mintApprovalToken('image_search', 'brave-search', 'nonce123');
    expect(
      verifyApprovalToken(t, 'other_tool', 'brave-search', 'nonce123'),
    ).toBe(false);
    expect(verifyApprovalToken(t, 'image_search', 'openai', 'nonce123')).toBe(
      false,
    );
    expect(
      verifyApprovalToken(t, 'image_search', 'brave-search', 'wrong'),
    ).toBe(false);
  });

  it('rejects a malformed (wrong-length) token without throwing', () => {
    expect(
      verifyApprovalToken('short', 'image_search', 'brave-search', 'nonce123'),
    ).toBe(false);
  });
});
