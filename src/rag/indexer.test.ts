...
describe('rag/indexer', () => {
  ...
  describe('indexText', () => {
    ...
    it('splits long text into multiple chunks', async () => {
      ...
      const { indexText } = await import('./indexer.js');
      await indexText(CFG, 'g', longText, 'document');
    ...
    it('generates stable deterministic IDs for the same content', async () => {
      ...
      await indexText(CFG, 'g', text, 'doc');
      const id1 = mockUpsertPoints.mock.calls[0][2][0].id;
      ...
      await indexText(CFG, 'g', text, 'doc');
      const id2 = mockUpsertPoints.mock.calls[0][2][0].id;
    ...
    it('generates different IDs for different groups with same text', async () => {
      ...
      const text =
        'same text that is definitely long enough to be indexed by the chunker correctly';
      await indexText(CFG, 'group-a', text, 'doc');
      const idA = mockUpsertPoints.mock.calls[0][2][0].id;

      vi.clearAllMocks();
      mockEmbed.mockImplementation(async (texts: string[]) =>
        texts.map(() => [0.1, 0.2, 0.3]),
      );
      await indexText(CFG, 'group-b', text, 'doc');
      const idB = mockUpsertPoints.mock.calls[0][2][0].id;
