...
describe('HttpChannel', () => {
  ...
  // ── POST /message — /cancel flows through onMessage ─────────────────────

  describe('POST /message — /cancel flows through onMessage', () => {
    it('calls onMessage for /cancel (shared intercept handles it, not HTTP layer)', async () => {
      const opts = makeOpts();
      const channel = new HttpChannel(makeConfig(), opts);
      await channel.connect();
      ...
      // /cancel reaches onMessage — the shared layer handles it, not HTTP
      expect(opts.onMessage).toHaveBeenCalledOnce();
      expect(opts.onMessage).toHaveBeenCalledWith(
        'http:alice',
        expect.objectContaining({ content: '/cancel', is_from_me: false }),
      );
      expect(res._status).toBe(200);
    ...
    it('does NOT intercept a normal message like "hello" differently', async () => {
      const opts = makeOpts();
      const channel = new HttpChannel(makeConfig(), opts);
      await channel.connect();
