...
export class RealPerGroupK8sClient implements PerGroupK8sClient {
  ...
  async applyService(s: V1Service): Promise<void> {
    ...
    if (existing) {
      ...
      await this.core.replaceNamespacedService({
        name,
        namespace: ns,
        body: s,
      });
  ...
  async applyNetworkPolicy(p: V1NetworkPolicy): Promise<void> {
    ...
    try {
      const existing = await this.net.readNamespacedNetworkPolicy({
        name,
        namespace: ns,
      });
      p.metadata = {
  ...
  async patchDeploymentReplicas(
    namespace: string,
    name: string,
    replicas: number,
  ): Promise<void> {
    // Use PUT on the scale subresource — replaceNamespacedDeploymentScale uses
    // application/json (PUT semantics) and does not require a JSON patch array,
    // unlike patchNamespacedDeploymentScale which defaults to json-patch+json.
  ...
  async deleteByLabel(namespace: string, labelSelector: string): Promise<void> {
    await this.apps.deleteCollectionNamespacedDeployment({
      namespace,
      labelSelector,
    });
    await this.core.deleteCollectionNamespacedService({
      namespace,
      labelSelector,
    });
    await this.net.deleteCollectionNamespacedNetworkPolicy({
      namespace,
      labelSelector,
    });
    await this.core.deleteCollectionNamespacedSecret({
      namespace,
      labelSelector,
    });
  }
  ...
  async waitForReady(
    ...
    throw new Error(
      `waitForReady: timeout after ${timeoutMs}ms for ${namespace}/${name}`,
    );
  }

  async listDeploymentsByLabel(
    namespace: string,
    labelSelector: string,
  ): Promise<V1Deployment[]> {
    const res = await this.apps.listNamespacedDeployment({
      namespace,
      labelSelector,
    });
    return res.items ?? [];
...
export class FakePerGroupK8sClient implements PerGroupK8sClient {
  ...
  async applyNetworkPolicy(p: V1NetworkPolicy): Promise<void> {
    this.store.policies.set(
      fakeKey(p.metadata!.namespace!, p.metadata!.name!),
      structuredClone(p),
    );
  }

  async applySecret(s: V1Secret): Promise<void> {
    this.store.secrets.set(
      fakeKey(s.metadata!.namespace!, s.metadata!.name!),
      structuredClone(s),
    );
  }
  ...
  async deleteByLabel(namespace: string, labelSelector: string): Promise<void> {
    const maps: Array<
      ...
      >
    > = [
      this.store.deployments as never,
      this.store.services as never,
      this.store.policies as never,
      this.store.secrets as never,
    ];
  ...
  async waitForReady(
    ...
    throw new Error(
      `waitForReady: timeout after ${timeoutMs}ms for ${ns}/${name}`,
    );
  }
  ...
  async listDeploymentsByLabel(
    ...
    for (const d of this.store.deployments.values()) {
      if (
        ...
        labelMatch(d.metadata?.labels, labelSelector)
      ) {
        out.push(d);
      }
    }
