...
describe('executeToolBridgeFile — per-field protocol', () => {
  ...
  it('writes declared fields and returns stdout on exit 0', async () => {
    shared = mkdtempSync(join(tmpdir(), 'fb-'));
    process.env.KUBECLAW_SHARED_DIR = shared;
    const call = executeToolBridgeFile(
      'bash',
      { command: 'echo hi', bogus: 'x' },
      'r1',
      ['command'],
    );
    const reqInput = join(shared, 'req', 'r1', 'input');
    for (let i = 0; i < 50 && !existsSync(reqInput); i++)
    ...
    const tmp = join(shared, '.resp.r1.tmp');
  ...
  it('returns an error containing stderr on non-zero exit', async () => {
    shared = mkdtempSync(join(tmpdir(), 'fb-'));
    process.env.KUBECLAW_SHARED_DIR = shared;
    const call = executeToolBridgeFile('bash', { command: 'boom' }, 'r2', [
      'command',
    ]);
    for (let i = 0; i < 50 && !existsSync(join(shared, 'req', 'r2')); i++)
      await new Promise((r) => setTimeout(r, 20));
    const tmp = join(shared, '.resp.r2.tmp');
    mkdirSync(tmp, { recursive: true });
