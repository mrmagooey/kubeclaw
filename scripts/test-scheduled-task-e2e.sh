...
# ── Step 0: Setup — register group, update secret, deploy mock, upgrade helm ─
# Register group BEFORE helm upgrade so the restarted orchestrator loads it at
# startup and has it in memory when scheduled tasks fire.
echo "[0/7] Setup: register group, deploy mock LLM, upgrade helm"
...
# Register the test user as a group in the orchestrator DB (before restart)
...
if echo "$result" | grep -qF "Registered group"; then
  ...
  echo "  stderr: $(cat /tmp/admin-tool-err.txt 2>/dev/null | head -5)"
fi

# Add test user to the secret (channel pod picks this up after helm upgrade restart)
...
# Deploy mock LLM server
bash "$(dirname "$0")/mock-llm-deploy.sh" kubeclaw

# Helm upgrade: restarts orchestrator (loads group from DB) + channel pod (picks up secret + mock URL)
...
# Wait for orchestrator to be ready (it loads group from DB on startup)
echo "  Waiting for orchestrator to be ready after helm upgrade..."
kubectl rollout status deployment/kubeclaw-orchestrator -n "$NS" --timeout=90s

# Explicitly restart channel pod so it picks up updated secret (users + mock LLM URL)
kubectl rollout restart deployment/kubeclaw-channel-http -n "$NS" >/dev/null 2>&1
echo "  Waiting for channel pod to restart with new credentials..."
if wait_for_pod "app=kubeclaw-channel-http" 60; then
  pass "Pods ready with mock LLM and registered group"
else
  fail "Channel pod restarted" "Ready within 60s" "timed out"
  exit 1
fi

# ── Step 1: (no-op — group and secret setup merged into step 0) ─────────────
echo "[1/7] Verify channel connectivity setup"
...
# Open a NEW SSE stream for task-fired in a separate file
curl --no-buffer -s -u "$TEST_USER:$TEST_PASS" "http://localhost:$HTTP_PORT/stream" > /tmp/task-fired-sse.txt 2>&1 &
FIRED_SSE_PID=$!
CLEANUP_PIDS+=($FIRED_SSE_PID)
sleep 2

echo "  Waiting up to 90s for task to fire (scheduler polls every 60s)..."
if wait_for_content /tmp/task-fired-sse.txt "TASK_FIRED_OK" 90; then
  kill $FIRED_SSE_PID 2>/dev/null || true
  sse_output=$(grep "TASK_FIRED_OK" /tmp/task-fired-sse.txt 2>/dev/null || true)
  pass "Scheduled task fired and executed"
  echo "    Output: $(echo "$sse_output" | head -1)"
else
  kill $FIRED_SSE_PID 2>/dev/null || true
  sse_output=$(grep '^data: ' /tmp/task-fired-sse.txt 2>/dev/null || true)
  if echo "$sse_output" | grep -q "TASK_FIRED_OK"; then
    pass "Scheduled task fired and executed"
    echo "    Output: $(echo "$sse_output" | grep "TASK_FIRED_OK" | head -1)"
  else
    fail "Task fired successfully" "TASK_FIRED_OK in SSE stream within 90s" "$(cat /tmp/task-fired-sse.txt 2>/dev/null | head -3)"
    echo "  Channel logs:"
    kubectl logs -l app=kubeclaw-channel-http -n "$NS" -c channel --tail=10 2>&1
  fi
fi
