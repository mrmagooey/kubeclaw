...
2. The orchestrator's `jobRunner.runToolJob()` (in `src/k8s/job-runner.ts`) creates a Kubernetes Job using the image returned by `getContainerImage(group.llmProvider)` (`src/config.ts`). All providers default to `kubeclaw-agent:latest`; override per-provider via `KUBECLAW_CONTAINER_IMAGE_<PROVIDER>` or globally via `KUBECLAW_CONTAINER_IMAGE`. The image is built from `container/agent-runner/`.
