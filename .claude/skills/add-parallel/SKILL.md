...
Adds Parallel AI MCP integration to KubeClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses KubeClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. KubeClaw already set up and running
...
Add Parallel AI MCP servers after the kubeclaw server:
```typescript
const mcpServers: Record<string, any> = {
  kubeclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__kubeclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__kubeclaw__send_message
   4. Use mcp__kubeclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo kubeclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep kubeclaw  # macOS
...
- Check task runs: `tail -f logs/kubeclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.kubeclaw` (macOS) or `systemctl --user restart kubeclaw` (Linux)
