---
name: x-integration
description: X (Twitter) integration for KubeClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** KubeClaw v1.0.0. Directory structure may change in future versions.
...
1. **KubeClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep kubeclaw (macOS) or systemctl --user status kubeclaw (Linux)
```
...
| `KUBECLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/kubeclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into KubeClaw, make the following modifications:
...
All paths below are relative to project root (`KUBECLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep kubeclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status kubeclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/kubeclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/kubeclaw.log | tail -20
```
...
docker run kubeclaw-agent ls -la /app/src/skills/
```
