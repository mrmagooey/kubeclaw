...
- Added: `gmail` MCP server alongside the existing `kubeclaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `kubeclaw` MCP server configuration is unchanged
...
## Must-keep
- The `kubeclaw` MCP server with its environment variables
